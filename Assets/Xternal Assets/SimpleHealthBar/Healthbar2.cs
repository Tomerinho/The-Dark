...
public class Healthbar2 : MonoBehaviour {
    ...
    private void Update()
    {
        ...
        {
            healthPercentage = int.Parse((Mathf.Round(maximumHealth * (health / 1000f))).ToString());
    ...
    public void UpdateHealth()
    {
        ...
        if (healthPercentage <= lowHealth && health >= minimumHealth && objFill.GetComponent<Image>().color != lowHealthColor)
        {
            ChangeHealthbarColor(lowHealthColor);
        }
        else if (healthPercentage <= highHealth && health > lowHealth)
        {
            float lerpedColorValue = (float.Parse(healthPercentage.ToString()) - 250) / 410;
        ...
        {
            float lerpedColorValue = (float.Parse(healthPercentage.ToString()) - 670) / 330;
